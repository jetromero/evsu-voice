...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['restore_suggestion'])) {
        ...
        $archive_stmt->execute([$archive_id]);
        ...
        if ($archived) {
            try {
                ...
                // Remove from archive
                $delete_archive_query = "DELETE FROM archived_suggestions WHERE id = ?";
                $delete_archive_stmt = $conn->prepare($delete_archive_query);
                $delete_archive_stmt->execute([$archive_id]);
        ...
        $delete_query = "DELETE FROM archived_suggestions WHERE id = ?";
        $delete_stmt = $conn->prepare($delete_query);

        if ($delete_stmt->execute([$archive_id])) {
...
try {
    ...
    $categories_query = "SELECT DISTINCT category FROM archived_suggestions ORDER BY category";
    $categories_stmt = $conn->prepare($categories_query);
    $categories_stmt->execute();
    $categories = $categories_stmt->fetchAll(PDO::FETCH_COLUMN);
}
