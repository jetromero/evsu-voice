...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['restore_suggestion'])) {
        ...
        $archive_query = "SELECT * FROM archived_suggestions WHERE id = ? AND user_id = ?";
        $archive_stmt = $conn->prepare($archive_query);
        $archive_stmt->execute([$archive_id, $user['id']]);
        $archived = $archive_stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($archived && (int)$archived['user_id'] === (int)$user['id']) {  // Double-check ownership
            try {
                ...
                $delete_archive_query = "DELETE FROM archived_suggestions WHERE id = ? AND user_id = ?";
                $delete_archive_stmt = $conn->prepare($delete_archive_query);
                $delete_archive_stmt->execute([$archive_id, $user['id']]);
        ...
        $delete_query = "DELETE FROM archived_suggestions WHERE id = ? AND user_id = ?";
        $delete_stmt = $conn->prepare($delete_query);

        if ($delete_stmt->execute([$archive_id, $user['id']])) {
            if ($delete_stmt->rowCount() > 0) {
                $_SESSION['my_trash_success'] = 'Suggestion permanently deleted!';
            } else {
                $_SESSION['my_trash_error'] = 'Suggestion not found or you do not have permission to delete it.';
            }
        } else {
...
$where_conditions = ["a.user_id = ?"];  // CRITICAL: Only show current user's suggestions
$params = [$user['id']];
...
if ($date_filter) {
    switch ($date_filter) {
        case 'today':
            $where_conditions[] = "DATE(a.deleted_at) = CURDATE()";
            break;
        case 'week':
            $where_conditions[] = "a.deleted_at >= DATE_SUB(NOW(), INTERVAL 1 WEEK)";
            break;
        case 'month':
            $where_conditions[] = "a.deleted_at >= DATE_SUB(NOW(), INTERVAL 1 MONTH)";
            break;
        case 'year':
            $where_conditions[] = "a.deleted_at >= DATE_SUB(NOW(), INTERVAL 1 YEAR)";
            break;
    }
}
...
$stmt->execute($params);
$archived_suggestions = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
try {
    ...
    $categories_query = "SELECT DISTINCT category FROM archived_suggestions WHERE user_id = ? ORDER BY category";
    $categories_stmt = $conn->prepare($categories_query);
    $categories_stmt->execute([$user['id']]);
    $categories = $categories_stmt->fetchAll(PDO::FETCH_COLUMN);
}
...
$stats_query = "SELECT
                ...
                FROM archived_suggestions WHERE user_id = ?";
