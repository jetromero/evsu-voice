...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['delete_suggestion'])) {
        ...
        $verify_query = "SELECT * FROM suggestions WHERE id = ? AND user_id = ?";
        $verify_stmt = $conn->prepare($verify_query);
        $verify_stmt->execute([$suggestion_id, $user['id']]);
        $suggestion_data = $verify_stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($suggestion_data) {
            try {
                ...
                $delete_query = "DELETE FROM suggestions WHERE id = ? AND user_id = ?";
                $delete_stmt = $conn->prepare($delete_query);
                $delete_stmt->execute([$suggestion_id, $user['id']]);
        ...
        $verify_query = "SELECT id FROM suggestions WHERE id = ? AND user_id = ? AND status = 'pending'";
        $verify_stmt = $conn->prepare($verify_query);
        $verify_stmt->execute([$suggestion_id, $user['id']]);

        if ($verify_stmt->fetch()) {
            if (!empty($title) && !empty($description) && !empty($category)) {
                $update_query = "UPDATE suggestions SET title = ?, description = ?, category = ?, is_anonymous = ?, updated_at = NOW() WHERE id = ? AND user_id = ?";
                $update_stmt = $conn->prepare($update_query);

                if ($update_stmt->execute([$title, $description, $category, $is_anonymous, $suggestion_id, $user['id']])) {
                                    ...
                                    <div class="action-buttons">
                                        <?php if ($suggestion['status'] === 'pending'): ?>
                                            <button type="button" class="action-btn update-btn"
                                                onclick="openUpdateModal(<?php echo $suggestion['id']; ?>, '<?php echo htmlspecialchars($suggestion['title'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($suggestion['category'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($suggestion['description'], ENT_QUOTES); ?>', <?php echo $suggestion['is_anonymous']; ?>)"
...
<script>
    ...
    function openUpdateModal(id, title, category, description, isAnonymous) {
        ...
        document.getElementById('update-anonymous').checked = isAnonymous == 1;
